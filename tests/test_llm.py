...
class TestContainerContinuation:
    async def test_container_id_passed_back_on_continuations(self):
        # Dynamic-filtering web tools run code execution in a container; a
        # continuation with pending code-exec tool uses must echo its id or
        # the API 400s ("container_id is required when there are pending tool
        # uses generated by code execution with tools").
        paused = _message([NS(type="text", text="searching", citations=None)], "pause_turn")
        paused.container = NS(id="container_abc")
        done = _message([NS(type="text", text="done", citations=None)], "end_turn")
        llm = _llm([paused, done])
        await llm.run_agentic(
            stage="s",
            system="s",
            messages=[{"role": "user", "content": "go"}],
        ...
        first, second = llm.client.calls
        assert "container" not in first
        assert second["container"] == "container_abc"

    async def test_container_id_passed_on_client_tool_follow_up(self):
        async def handler(tool_input):
            return ToolOutcome(content="ok")

        mixed = _message([_tool_use("tu1", "hn_search", {"query": "a"})], "tool_use")
        mixed.container = NS(id="container_xyz")
        done = _message([NS(type="text", text="done", citations=None)], "end_turn")
        llm = _llm([mixed, done])
        await llm.run_agentic(
            stage="s",
        ...
        assert llm.client.calls[1]["container"] == "container_xyz"

    async def test_no_container_kwarg_when_absent(self):
        done = _message([NS(type="text", text="done", citations=None)], "end_turn")
        llm = _llm([done])
        await llm.run_agentic(
            stage="s",
